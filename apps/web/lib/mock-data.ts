...
// 실제 서울 전역 191개 매장(카카오 로컬 API 조회 결과, apps/api/prisma/data/
// seoul-escape-rooms.json)은 목업 모드의 인메모리 픽스처에는 미러링하지 않는다
// — 위 4개 매장처럼 테마 상세를 알아야 의미 있는 화면이 되는데, 매장 단위
// 정보만 있는 191곳을 그대로 복제하면 실연동 모드와 달리 전부 뻔한 플레이스홀더
// 카드로만 채워져 목업 모드의 데모 가치가 없다. 목업 모드는 실연동 모드와
// 별개로 소수의 검증된 데모 데이터만 유지한다.
